// Typed UI-side client for the native application-command seam.
//
// Wraps the shell's `appCommand` RPC so views can issue commands by method name
// with fully typed params and results. The Bun shell forwards each request into
// the native core (see src/bun/index.ts + src/bun/nativeCore.ts).
