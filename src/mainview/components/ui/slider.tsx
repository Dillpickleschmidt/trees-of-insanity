...
export const Slider: Component<SliderProps> = (props) => {
	...
	return (
		<KobalteSlider
			class={cn("relative flex w-full touch-none select-none flex-col items-center gap-2", local.class)}
			{...(local.defaultValue !== undefined ? { defaultValue: local.defaultValue } : {})}
			{...rest}
		>
			{local.label && (
				<div class="flex w-full justify-between">
					<KobalteSlider.Label class="text-sm font-medium leading-none">{local.label}</KobalteSlider.Label>
