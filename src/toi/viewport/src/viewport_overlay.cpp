...
Result<ViewportOverlay> ViewportOverlay::create(VulkanContext& context, VkFormat swapchain_format)
{
...
Result<void> ViewportOverlay::set_swapchain(VulkanContext& context, const VulkanSwapchain& swapchain)
{
    reset_swapchain();
    const auto& images = swapchain.images();
    image_views_.resize(images.size(), VK_NULL_HANDLE);
    ...
    for (std::size_t index = 0; index < images.size(); ++index) {
        ...
        if (auto result = require_vk(vkCreateImageView(device_, &view_info, nullptr, &image_views_[index]),
                                     "vkCreateImageView");
            !result) {
            return std::unexpected(result.error());
        }
        ...
        framebuffer_info.pAttachments = &image_views_[index];
        framebuffer_info.width = extent.width;
        framebuffer_info.height = extent.height;
        framebuffer_info.layers = 1;
        if (auto result = require_vk(vkCreateFramebuffer(device_, &framebuffer_info, nullptr, &framebuffers_[index]),
                                     "vkCreateFramebuffer");
            !result) {
            return std::unexpected(result.error());
        }
                                     ...
                                     std::uint32_t distance_slot)
{
    ...
    begin.framebuffer = framebuffers_[image_index];
...
void ViewportOverlay::reset_swapchain()
{
