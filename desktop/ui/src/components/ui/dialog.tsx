...
const DialogTitle: Component<ComponentProps<typeof KobalteDialog.Title>> = (props) => {
	const [local, rest] = splitProps(props, ["class"]);
	return <KobalteDialog.Title class={cn("text-lg font-semibold leading-none tracking-tight", local.class)} {...rest} />;
};

const DialogDescription: Component<ComponentProps<typeof KobalteDialog.Description>> = (props) => {
	const [local, rest] = splitProps(props, ["class"]);
	return <KobalteDialog.Description class={cn("text-sm text-muted-foreground", local.class)} {...rest} />;
};
