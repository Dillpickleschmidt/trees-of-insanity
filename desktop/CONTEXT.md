...
- **Flow overlay**: Plant-workspace Vulkan surface-material effect showing light toward the root and vigor toward developed frontiers. Animated circumferential bands reuse the rendered pipe surface, use a blue-cyan-green-yellow-red map for root-normalized flow fraction, and visualize the current committed plant state rather than the preceding transition.
- **Plant diagnostic controls**: independent toggles for module diagnostic labels, direct-light bounding spheres, accumulated-light flow, vigor flow, and mature-terminal markers. Mature terminals render filled when occupied and hollow when unoccupied, with color distinguishing the main-axis terminal from lateral terminals.
