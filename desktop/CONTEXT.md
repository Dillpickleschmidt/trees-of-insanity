...
- **Flow overlay**: Plant-workspace GPU-animated dashed ribbons showing light toward the root and vigor toward terminals. Both use a blue-cyan-green-yellow-red weight map for their root-normalized flow fraction, while world-space ribbon width matches the current host-segment diameter. It shows only flow that occurred in a completed plant step, never predicted or inactive routes.
...
- **Diagnostic rendering**: a graphics-owned overlay module accepts generic batched world lines, branch-surface paths, markers, and translucent spheres; it contains no Plant-workspace semantics. Plant render projection supplies those primitives and native-projected label anchors, while module diagnostic labels use Solid DOM elements. Bounding spheres receive stable module colors in projection and render as analytic, depth-aware sphere impostors.
