...
- Milestone 5: provisional shared `v̄_min = 0.02`; freed-terminal reuse; deterministic runs; no observed attach/shed oscillation.
- Milestone 6: root-vigor senescence decline remains deferred.

## Approved project decisions

1. **Mature attachment:** evaluate every mature unoccupied terminal each step; a newly mature module is first evaluated in its crossing step. ADR 0018 supersedes crossing-only ADR 0012.
2. **Eligibility state:** attachment occupancy and light/vigor come from the complete pre-step state, including for modules that cross maturity; post-integration evaluation supplies attachment geometry only.
3. **Vacated-terminal timing:** a terminal vacated by shedding is first eligible in the following step.
4. **Threshold calibration:** retain `v̄_min = 0.02` and continue labeling it provisional.
...
7. **Oscillation response:** representative bundled runs exposed repeated same-terminal attach/shed cycles under stateless reuse. ADR 0019 therefore adds a same-threshold recovery latch: a vacated terminal must first be observed at/below `v̄_min` while empty, then later rise above it. No cooldown, second threshold, or age protection is added.
