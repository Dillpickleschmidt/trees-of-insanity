...
The plant carries one vigor quantity, seeded at the root by `0009-root-vigor-follows-accumulated-light.md` and divided among child modules by Eq. 2 without loss, so a module's vigor may exceed `v̄_max`. The paper's instruction to clamp `v̄(u)` to `v̄_max` is a normalization, applied only where a bounded value is required: Eq. 5, whose sigmoid argument `(v̄ − v̄_min)/(v̄_max − v̄_min)` must lie in `[0, 1]`, and morphospace `D′ = v̄(u_parent) · D / v̄_max`, which only maps into `[0, D]` when its numerator is bounded. Truncating the propagated flux instead would discard vigor at every module junction and make `v̄_rootmax` unreachable.
